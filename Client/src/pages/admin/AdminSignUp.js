...
const AdminSignUp = () => {
  ...
  const handleSubmit = async (event) => {
    event.preventDefault();
      const { password, name, username } = values;
    ...
    <div>
      <div className="w-full min-h-screen flex items-center justify-center bg-[#2c3e50]">
        <form
          ...
          <h1 className="text-white text-center text-2xl mb-3 mt-5 font-bold">Sign Up</h1>
          <input
            type="text"
            name="name"
            placeholder="Name"
          ...
          <span className="text-white mb-4">
            already have a account ? <Link to="/admin/signin" className="text-[#4e0eff]">Login</Link>{" "}
          </span>
        </form>
      </div>
    </div>
